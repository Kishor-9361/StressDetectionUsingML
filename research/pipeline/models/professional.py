...
class HybridMoEAttentionModel(nn.Module):
    def __init__(self, hidden_dim=16, num_subjects=65, adversarial=False, dual_representation=False,
                 eye_dim=10, mouth_dim=6, gface_dim=18,
                 prosody_dim=7, spectral_dim=5, quality_dim=12,
                 cardio_dim=8, motion_dim=6):
        super().__init__()
        self.adversarial = adversarial
        self.dual_representation = dual_representation
        
        # Sub-modality specialized experts
        self.exp_eye = SequenceEncoder(input_dim=eye_dim, hidden_dim=hidden_dim)
        self.exp_mouth = SequenceEncoder(input_dim=mouth_dim, hidden_dim=hidden_dim)
        self.exp_global_face = SequenceEncoder(input_dim=gface_dim, hidden_dim=hidden_dim)
        
        self.exp_prosody = SequenceEncoder(input_dim=prosody_dim, hidden_dim=hidden_dim)
        self.exp_spectral = SequenceEncoder(input_dim=spectral_dim, hidden_dim=hidden_dim)
        self.exp_quality = SequenceEncoder(input_dim=quality_dim, hidden_dim=hidden_dim)
        
        self.exp_cardio = SequenceEncoder(input_dim=cardio_dim, hidden_dim=hidden_dim)
        self.exp_motion = SequenceEncoder(input_dim=motion_dim, hidden_dim=hidden_dim)
